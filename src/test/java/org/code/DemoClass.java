...
public class DemoClass extends BaseClass {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * "C:\\Users\\impelox-pc-048\\eclipse-workspace\\Demo\\target\\demo - Copy.xlsx"
		 * ;
		 * 
		 * Connection connection = null;
		 * 
		 * try { // Connect to the database connection =
		 * DriverManager.getConnection(dbURL, dbUsername, dbPassword); String query =
		 * "SELECT * FROM 7003_group_medical_aiaw_transactions.group where status = 1;";
		 * Statement statement = connection.createStatement(); ResultSet resultSet =
		 * statement.executeQuery(query);
		 * 
		 * // Open the Excel file FileInputStream fileInputStream = new
		 * FileInputStream(new File(excelFilePath)); Workbook workbook = new
		 * XSSFWorkbook(fileInputStream); Sheet sheet = workbook.getSheetAt(0); //
		 * Modify index if the data needs to go to a specific sheet int rowCount =
		 * sheet.getLastRowNum();
		 * 
		 * // Write data to the Excel sheet while (resultSet.next()) { Row row =
		 * sheet.createRow(++rowCount); for (int i = 1; i <=
		 * resultSet.getMetaData().getColumnCount(); i++) { Cell cell = row.createCell(i
		 * - 1); cell.setCellValue(resultSet.getString(i)); String stringCellValue =
		 ...
		 * 
		 * // Save the changes to the Excel file FileOutputStream fileOutputStream = new
		 * FileOutputStream(new File(excelFilePath)); workbook.write(fileOutputStream);
